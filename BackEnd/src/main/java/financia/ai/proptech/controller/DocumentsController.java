...
public class DocumentsController {
    ...
    @PostMapping
    public ResponseEntity<DocumentsDto> create(@RequestBody DocumentsDto docsDto){
    ...
    public ResponseEntity<DocumentsDto> getDoc(@PathVariable Long id){
        return  docsService.documentsId(id).map(doc -> new ResponseEntity<>(doc, HttpStatus.OK))
                .orElse(new ResponseEntity<>(HttpStatus.NOT_FOUND));
    }

    @PutMapping("/{id}")
    public ResponseEntity<DocumentsDto> updateDocs(@PathVariable Long id, @RequestBody DocumentsDto docsDto){
        return docsService.update(id, docsDto).map(doc -> new ResponseEntity<>(doc, HttpStatus.ACCEPTED))
                .orElse(new ResponseEntity<>(HttpStatus.NOT_FOUND));
    }
